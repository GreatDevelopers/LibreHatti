...
def programmeletter(request):
    ...
    buyer_name = User.objects.values('first_name','last_name','id').get(id=old_post['buyer'])
    street_address = Address.objects.values('street_address','city','pin','province').get(id=buyer_name['id'])
    old_post = request.session.get('old_post')
    ...
    return render(request, 'programmeletter/programmeletter.html',
        ...
        'staffinteam':staffinteam, 'buyer':buyer_name,'address':street_address})
