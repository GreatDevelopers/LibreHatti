...
class LabReportForm(forms.Form):
    ...
    filter(parent__parent__isnull=True).filter(parent__isnull=False), label = _PARENT_CATEGORY)
    try:
        ...
        sub_category = forms.ChoiceField(sub_category_choices, label = _SUB_CATEGORY)
    except:
        pass
