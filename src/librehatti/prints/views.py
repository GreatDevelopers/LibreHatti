...
def bill(request):
    ...
    purchased_item_obj = (
        PurchasedItem.objects.filter(purchase_order__in=purchase_order)
        .values("item__name", "item__category", "qty", "price_per_unit")
        .order_by("item__category")
    )
    cost = (
        PurchasedItem.objects.filter(purchase_order__in=purchase_order)
...
def suspense_bill(request):
    ...
    id = request.GET["order_id"]
    voucherid = (
        VoucherId.objects.filter(purchase_order=id)
        .values("voucher_no", "purchased_item__item__category", "session")
        .distinct()
    )
...
def tax(request):
    ...
    bill = Bill.objects.values(
        "totalplusdelivery", "total_tax", "purchase_order__date_time"
    ).get(purchase_order=id)
...
def receipt(request):
    ...
    if not voucherid_temp["receipt_no_of_session"]:
        ...
        for value in financialsession:
            ...
            if start_date <= today_date <= end_date:
                session_id = value["id"]
        max_receipt_no = VoucherId.objects.filter(session=session_id).aggregate(
            Max("receipt_no_of_session")
        )
    ...
    voucherid = VoucherId.objects.values(
        "purchase_order_of_session", "receipt_no_of_session"
    ).filter(purchase_order=id)[0]
...
def quoted_bill(request):
    ...
    quoted_item = (
        QuotedItem.objects.filter(quoted_order=quoted_order_id)
        .values(
            "item__category__name",
            "item__category",
            "item__category__unit",
