    ...
    try:
        parent_category = forms.ModelChoiceField(
            queryset=Category.objects.filter(
                parent__parent__isnull=True).filter(parent__isnull=False),
...
class SpecialCategoriesForm(forms.ModelForm):
    ...
    category = forms.ModelChoiceField(queryset=Category.objects.filter(
        parent__parent__parent__isnull=True).filter(
        parent__isnull=False).filter(parent__parent__isnull=False))
...
class ProductListForm(forms.Form):
    ...
    select_lab = forms.ModelChoiceField(queryset=Category.objects.filter(
        parent__isnull=True))
