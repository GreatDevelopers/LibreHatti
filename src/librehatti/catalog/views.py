...
def bill_cal(request):
    ...
    purchase_order_obj = PurchaseOrder.objects.values(
        'total_discount', 'tds').get(id=purchase_order_id)
    purchase_item = PurchasedItem.objects.filter(
        purchase_order=purchase_order_id).aggregate(Sum('price'))
    ...
    if distance['distance_estimated__sum']:
        delivery_charges = int(distance[
                                   'distance_estimated__sum'])*delivery_rate[
            0]['value']
    ...
    if taxes_applied_temp:
        taxes_applied_obj = TaxesApplied.objects.filter(
            purchase_order=purchase_order_id).aggregate(Sum('tax'))
...
def previous_value(request):
    ...
    voucher_no = VoucherId.objects.values('voucher_no', 'session').filter(
        purchase_order=purchase_order_id)
    for value in voucher_no:
        CalculateDistribution.objects.get(
            voucher_no=value['voucher_no'], session=value['session']).delete()
...
def order_added_success(request):
    ...
    details = VoucherId.objects.values('purchase_order__buyer__first_name',
                                       ...
                                       'receipt_no_of_session').filter(
        purchase_order=order_id)[0]
    suspense_flag = 0
    ...
    return render(request, 'catalog/order_added_success.html',
                  {'details': details, 'order_id': order_id,
...
def change_request(request):
    ...
    if request.method == 'POST':
        ...
        object = VoucherId.objects.filter(session_id=session).filter(
        ...
        if object:
            ...
            surcharge = TaxesApplied.objects.values(
                'surcharge__tax_name','id', 'tax').filter(
            ...
            details = VoucherId.objects.values(
                ...
                'purchase_order__mode_of_payment__method',
                'purchase_order__cheque_dd_number',
                'purchase_order__cheque_dd_date').filter(
                purchase_order_of_session=purchase_order_of_session)[0]
            ...
            return render(request, 'catalog/change_form.html',
                          {'details': details, 'order_id':
                           ...
                           'session': session, 'surcharge': surcharge,
                           'bill': bill, 'messages': messages, 'request':
                               request_status})
        else:
        ...
        form = ChangeRequestForm()
        request_status = request_notify()
        return render(request, 'catalog/change_request.html', {'form': form,
                                                               'request':
                                                                   request_status})
...
def nonpaymentorderofsession(request):
    ...
    try:
        NonPaymentOrderOfSession.objects.values(
            'non_payment_order_of_session').get(
            non_payment_order=nonpaymentorder_id)
    except:
        session_id = None
        ...
        if max_id['id__max'] is None:
            obj = NonPaymentOrderOfSession(non_payment_order=nonpaymentorderobj,
                                           non_payment_order_of_session=1,
                                           session=session)
            obj.save()
        else:
            nonpayobj = NonPaymentOrderOfSession.objects.values(
                'non_payment_order_of_session', 'session').get(
                id=max_id['id__max'])
    ...
    return HttpResponseRedirect(reverse(
        "librehatti.catalog.views.nonpaymentordersuccess"))
...
def nonpaymentordersuccess(request):
    ...
    details = NonPaymentOrder.objects.values('buyer__first_name',
                                             ...
                                             'buyer__customer__address__district').filter(id=nonpaymentorder_id)[0]
