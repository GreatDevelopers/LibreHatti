...
def jsreverse(request):
    ...
    return HttpResponse(reverse_lazy(string_to_reverse))
...
def bill_cal(request):
    ...
    return HttpResponseRedirect(reverse_lazy("catalog:order_added_success"))
...
def previous_value(request):
    ...
    for value in voucher_no:
        CalculateDistribution.objects.get(
            voucher_no=value["voucher_no"], session=value["session"]
        ).delete()
    VoucherId.objects.filter(purchase_order=purchase_order_id).delete()
    return HttpResponseRedirect(reverse_lazy("voucher:voucher_generate"))
...
def order_added_success(request):
    ...
    ).filter(purchase_order=order_id)[0]
    suspense_flag = 0
    ...
    return render(
        request,
        "catalog/order_added_success.html",
        {
            "details": details,
            "order_id": order_id,
...
def change_request(request):
    ...
    if request.method == "POST":
        ...
        object = list(
            VoucherId.objects.filter(session_id=session)
...
def nonpaymentorderofsession(request):
    ...
    try:
        ...
        if max_id["id__max"] is None:
            ...
            nonpayobj = NonPaymentOrderOfSession.objects.values(
                "non_payment_order_of_session", "session"
    ...
    return HttpResponseRedirect(reverse_lazy("catalog:nonpaymentordersuccess"))
...
def nonpaymentordersuccess(request):
    ...
    old_post = request.session.get("old_post")
    nonpaymentorder_id = request.session.get("nonpaymentorder_id")
    details = NonPaymentOrder.objects.values(
        "buyer__first_name",
        "buyer__last_name",
    ...
    return render(
        request,
        "catalog/nonpaymentsuccess.html",
        {"data": old_post, "details": details, "order_id": non_pay_order_id},
    )
