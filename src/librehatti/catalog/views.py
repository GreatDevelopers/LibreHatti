...
"""
This view allows to calculate taxes on purchased order and calculating bill and saving these values in database
"""
def bill_cal(request):
    ...
    purchase_item = PurchasedItem.objects.filter(purchase_order=purchase_order_id).aggregate(Sum('price'))
    ...
    for value in surcharge:
        ...
        taxes_applied = TaxesApplied(purchase_order = purchase_order, surcharge = surcharge_obj, tax = taxes)
        taxes_applied.save()
    taxes_applied_obj = TaxesApplied.objects.filter(purchase_order=purchase_order_id).aggregate(Sum('tax'))
    tax_total = taxes_applied_obj['tax__sum']
    grand_total = price_total + tax_total
    bill = Bill(purchase_order = purchase_order, total_cost = price_total, total_tax = tax_total, grand_total = grand_total)
    bill.save()
