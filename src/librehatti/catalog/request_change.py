    ...
    if request.method == 'POST':
        ...
        try:
            ...
            ChangeRequest.objects.filter(
                ...
                purchase_order_of_session=purchase_order_of_session,session =
                session, previous_total=previous_total,new_total=new_total,
                description=description, initiator=full_name)
        except:
            obj= ChangeRequest(
                purchase_order_of_session=purchase_order_of_session,
        ...
        try:
            ...
            RequestStatus.objects.filter(
                change_request=change_request).update(
                change_request=change_request,confirmed=0,cancelled=0,
...
def list_request(request):
    ...
    for value in request_list:
        if RequestStatus.objects.filter(change_request = value['id']).filter(
                confirmed=False).filter(cancelled=False):
            request_status = 'Waiting'
        elif RequestStatus.objects.filter(change_request = value[
            'id']).filter(confirmed=True):
            request_status = 'Confirmed'
        elif RequestStatus.objects.filter(change_request = value[
            'id']).filter(cancelled=True):
...
def view_request(request):
    ...
    previous_total = ChangeRequest.objects.values('previous_total').filter(
        id = request_id)[0]
    new_total = ChangeRequest.objects.values('new_total').filter(
        id = request_id)[0]
    description = ChangeRequest.objects.values('description').filter(
        id = request_id)[0]
    initiator = ChangeRequest.objects.values('initiator').filter(
        id = request_id)[0]
    initiation_date = ChangeRequest.objects.values('initiation_date').filter(
        id = request_id)[0]
    surcharge_diff = RequestSurchargeChange.objects.values(
        'surcharge__surcharge__tax_name','previous_value','new_value').filter(
        change_request=request_id)
    if RequestStatus.objects.filter(change_request = request_id).filter(
            confirmed=False).filter(cancelled=False):
        request_status = 'Waiting'
    ...
    try:
        request_response = RequestStatus.objects.values(
            'request_response').filter(change_request = request_id)[0]
    except:
        pass
...
def reject_request(request):
    ...
    voucherid = VoucherId.objects.values('purchase_order').filter(
        purchase_order_of_session=change_request_obj[
            'purchase_order_of_session'], session_id=change_request_obj[
            'session_id'])[0]
    Bill.objects.filter(purchase_order=voucherid['purchase_order']).update(
