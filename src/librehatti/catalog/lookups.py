...
class BuyerLookup(LookupChannel):
    ...
    def get_query(self, q, request):
        ...
        for value in q.split():
            user = user.filter(Q(username__icontains=value) | Q(
                first_name__icontains=value) | Q(last_name__icontains=value)
                               | Q(
                customer__address__street_address__icontains=value) | Q(
                customer__address__district__icontains=value) | Q(
                customer__address__province__icontains=value) | Q(
                customer__title__icontains=value) | Q(
                customer__company__icontains=value))
        return user[0:15]
    ...
    def format_item_display(self, obj):
        result = User.objects.values('first_name', 'last_name',
                                     ...
                                     'customer__company').filter(id=obj.id)[0]
        return "<b>Name or Title:</b> %s <br> <b>Company:</b> %s <br> " \
               "<b>Address:</b> %s <br> %s <hr>" % ((result['first_name']
                                                     + ' ' + result['last_name']
