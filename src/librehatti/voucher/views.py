...
def voucher_generate(request):
    ...
    purchased_item_id = (
        PurchasedItem.objects.values_list("id", flat=True)
        .filter(purchase_order=purchase_order_id)
        .order_by("item__category")
    )
    voucherno = 0
    ...
    if max_id["id__max"] == None:
        ...
        for value in purchased_item:
            if generate_voucher == 0:
                voucherno = 0
                is_special_var = 1
            distribution_type = CategoryDistributionType.objects.values(
                "distribution"
            ).get(category=purchased_item[item])
            distribution = Distribution.objects.values(
                "id", "ratio", "college_income", "admin_charges"
            ).get(id=distribution_type["distribution"])
            distribution_id = Distribution.objects.get(
        ...
        if voucherid["voucher_no"] == 0:
            temp_obj = VoucherId.objects.values("id").filter(
                is_special=0, session=session
            )
            if temp_obj:
    ...
    voucher_obj1 = (
        VoucherId.objects.values_list("purchased_item", flat=True)
        .filter(purchase_order=purchase_order_id)
        .order_by("id")
    )
    voucher_obj2 = (
        VoucherId.objects.values_list("voucher_no", flat=True)
        .filter(purchase_order=purchase_order_id)
        .order_by("id")
    )
    i = 0
    ...
    for value in voucher_obj:
        ...
        try:
            ...
            if voucher_obj2[i] == voucher_obj2[i + 1]:
                ...
                distribution_obj = Distribution.objects.values(
                    "ratio", "college_income", "admin_charges"
                ).get(id=distribution_type["distribution"])
                cost_college_income = round(
                    (price * distribution_obj["college_income"]) / 100
                )
                cost_admin_charges = round(
                ...
                voucher_total = VoucherTotal(
                    voucher_no=voucher_number, session=session, total=price
                )
                voucher_total.save()
                price = 0
                flag = 1
    ...
    return HttpResponseRedirect(reverse("suspense:add_distance"))
...
def voucher_show(request):
    ...
    temp_voucherid = VoucherId.objects.values("voucher_no").filter(
    ...
    return render(
        ...
        {
            "voucherid": voucher_obj_distinct,
            "suspense_order": suspense_order,
...
def voucher_print(request):
    ...
    emp = (
        Staff.objects.values("name", "position__position")
        .filter(lab=lab_id)
        .filter(always_included=1)
    ...
    taxes_applied = TaxesApplied.objects.values(
        "surcharge__tax_name", "surcharge__value", "tax"
    ).filter(purchase_order=purchase_order_id)
