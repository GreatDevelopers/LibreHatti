...
import librehatti.voucher.models

from librehatti.catalog.models import PurchaseOrder, PurchasedItem, Bill,\
    HeaderFooter, TaxesApplied,SpecialCategories
...
def voucher_generate(request):
    ...
    financialsession = librehatti.voucher.models.FinancialSession.objects.values(
    ...
    for value in financialsession:
        ...
        if start_date <= purchase_order_date <= end_date:
            session_id = value['id']
    purchased_item = PurchasedItem.objects.values_list(
        'item__category', flat=True).filter(
        purchase_order = purchase_order_id).order_by('item__category')
    purchased_item_id = PurchasedItem.objects.values_list('id',
    flat=True).filter(purchase_order = purchase_order_id).order_by(
        'item__category')
    item = 0
    session = librehatti.voucher.models.FinancialSession.objects.get(id = session_id)
    poi = PurchaseOrder.objects.get(id = purchase_order_id)
    max_id = librehatti.voucher.models.VoucherId.objects.all().aggregate(Max('id'))
    if max_id['id__max'] == None:
        ...
        for value in purchased_item:
            if generate_voucher == 0:
                voucherno = 0
                is_special_var = 1
            distribution_type = librehatti.voucher.models.CategoryDistributionType.objects.values(
                'distribution').get(category = purchased_item[item])
            distribution = librehatti.voucher.models.Distribution.objects.values(
                'id','ratio','college_income','admin_charges').get(
                id = distribution_type['distribution'])
            distribution_id = librehatti.voucher.models.Distribution.objects.get(id = distribution_type['distribution'])
            voucherid = librehatti.voucher.models.VoucherId(purchase_order = poi,
        ...
        voucherid = librehatti.voucher.models.VoucherId.objects.values('voucher_no',
        'purchase_order_of_session', 'purchased_item_of_session',
        'session').get(id = max_id['id__max'])
        if voucherid['voucher_no'] == 0:
            temp_obj = librehatti.voucher.models.VoucherId.objects.values('id').filter(is_special=0,
                                                                                       session=session)
            if temp_obj:
                for temp_val in temp_obj:
                    maxid = temp_val['id']
                voucherid_temp = librehatti.voucher.models.VoucherId.objects.values('voucher_no').get(
                    id=maxid)
        ...
        purchase_order_of_session = voucherid['purchase_order_of_session'] + 1
        purchased_item_of_session = voucherid['purchased_item_of_session']
        if session_id == pre_purchase_order_session:
            ...
            for value in purchased_item:
                ...
                distribution = librehatti.voucher.models.Distribution.objects.values(
                    'id','ratio','college_income','admin_charges').get(
                    id = distribution_type['distribution'])
                distribution_id = librehatti.voucher.models.Distribution.objects.get(
                    id = distribution_type['distribution'])
                voucherid = librehatti.voucher.models.VoucherId(purchase_order = poi,
                                                                purchased_item = PurchasedItem.objects.get(
                    pk = purchased_item_id[item]), voucher_no = voucherno,
            ...
            for value in purchased_item:
                ...
                voucherid = librehatti.voucher.models.VoucherId(purchase_order = poi,
                    ...
                    pk = purchased_item_id[item]), voucher_no = voucherno,
                                      ...
                                      purchaseditemofsession, session = session,
                                                                distribution = distribution_id,
                                                                ratio = distribution['ratio'],
                                                                college_income = distribution[
                                          'college_income'],
                                                                admin_charges = distribution[
                                          'admin_charges'],
                                                                is_special=is_special_var)
                voucherid.save()
                try:
                    ...
                    continue
    voucher_obj = librehatti.voucher.models.VoucherId.objects.values_list('id', flat=True).filter(
        purchase_order = purchase_order_id).order_by('id')
    voucher_obj1 = librehatti.voucher.models.VoucherId.objects.values_list(
        'purchased_item', flat=True).filter(
        purchase_order = purchase_order_id).order_by('id')
    voucher_obj2 = librehatti.voucher.models.VoucherId.objects.values_list(
        'voucher_no', flat=True).filter(
        purchase_order = purchase_order_id).order_by('id')
    i = 0
    price = 0
    flag = 0
    for value in voucher_obj:
        if generate_voucher == 0:
            break
        purchased_item_obj = PurchasedItem.objects.values(
            'price','item__category').get(
            purchase_order = purchase_order_id,id = voucher_obj1[i])
        try:
            if voucher_obj2[i] == voucher_obj2[i+1]:
                ...
                distribution_obj = librehatti.voucher.models.Distribution.objects.values(
                    'ratio', 'college_income', 'admin_charges').get(
                    id = distribution_type['distribution'])
                cost_college_income = round((price * distribution_obj[
                    'college_income'])/100)
                cost_admin_charges = round((price * distribution_obj[
                    'admin_charges'])/100)
                ...
                calculate_distribution = librehatti.voucher.models.CalculateDistribution(
                ...
                voucher_total = librehatti.voucher.models.VoucherTotal(voucher_no=voucher_number,
            ...
            if flag == 0:
                ...
                price = price + price_item
                voucher_number = voucher_obj2[i]
                distribution_type = librehatti.voucher.models.CategoryDistributionType.objects.\
                values('distribution').\
...
def voucher_show(request):
    ...
    temp_voucherid = librehatti.voucher.models.VoucherId.objects.values('voucher_no').filter(
        purchase_order=purchase_order)[0]
    message = 'Voucher'
    if temp_voucherid['voucher_no'] == 0:
        message = "No voucher to display"
    voucherid = librehatti.voucher.models.VoucherId.objects.values('purchase_order', 'purchased_item',
        'voucher_no', 'session','purchase_order_of_session').filter(
    ...
    suspense_order = SuspenseOrder.objects.values(
        'voucher', 'session_id_id').filter(purchase_order=id, is_cleared=1)
...
def voucher_print(request):
    ...
    if suspense_order:
        flag = 1
    calculatedistribution = librehatti.voucher.models.CalculateDistribution.objects.values(
    ...
    voucherid = librehatti.voucher.models.VoucherId.objects.values('purchase_order', 'ratio',
    ...
    'purchased_item__item__category__parent').filter(
        voucher_no = number, session = session)[0]
    ...
    emp = Staff.objects.values('name','position__position').filter(
        lab=lab_id).filter(always_included=1).order_by(
    ...
    purchase_order_obj = PurchaseOrder.objects.values(
        'date_time', 'buyer','buyer__first_name','buyer__last_name',
        'tds','buyer__customer__title','buyer__customer__company').get(
        id = purchase_order)
    address = Customer.objects.values(
        'address__street_address','address__district', 'address__pin',
        'address__province').get(user = purchase_order_obj['buyer'])
    date = purchase_order_obj['date_time']
    bill = Bill.objects.values('delivery_charges','total_cost',
        'grand_total','amount_received','totalplusdelivery').get(
        purchase_order = purchase_order_id)
    ...
    voucheridobj = librehatti.voucher.models.VoucherId.objects.values('purchase_order_of_session',
        'purchase_order__mode_of_payment__method',
        'purchase_order__cheque_dd_number','purchase_order__cheque_dd_date',
        'purchase_order__mode_of_payment').filter(
        purchase_order=purchase_order_id)[0]
