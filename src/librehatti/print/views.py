...
from librehatti.catalog.models import Product, PurchaseOrder,PurchasedItem
...
def add_material(request):
  if request.method == 'POST':
    ...
    purchase_item= PurchasedItem.objects.filter(purchase_order__date_time__range
                  =(start_date,end_date),item__name=material).values_list( 
    ...
    material_name = Product.objects.values('name')
