...
def add_distance(request):
    ...
    for item in items:
        if item:
            parents.append(
                PurchasedItem.objects.values("item__category__parent__name", "id")
                .filter(item=item)
        ...
        for category in parent:
            ...
            try:
                if (
                    ...
                    or temp_val[1].upper() == " FIELD WORK"
                ):
    ...
    if field_work and generate_voucher == 1:
        if request.method == "POST":
            ...
            return HttpResponseRedirect(reverse("catalog:bill_cal"))
        else:
            ...
            voucher = (
                VoucherId.objects.values(
                    "voucher_no", "purchased_item__item__category__name"
                )
                .filter(purchase_order=purchase_order_id)
        ...
        voucher = (
            VoucherId.objects.values("voucher_no")
            .filter(purchase_order=purchase_order_id)
            .distinct()
        )
...
def clearance_search(request):
    ...
    if request.method == "POST":
        ...
        if sessiondata.is_valid():
            ...
            object = list(
                SuspenseOrder.objects.filter(session_id=session)
            ...
            if object:
                ...
                request_status = request_notify()
                temp = {"form": form, "message": errors, "request": request_status}
...
def clearance_result(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            suspense_object = SuspenseOrder.objects.filter(
                voucher=voucher_no, session_id=session
            ).update(is_cleared=0)
            try:
                SuspenseClearance.objects.get(
            ...
            form = Clearance_form(
                ...
                initial={
                    "voucher_no": voucher,
                    "session": session,
                    "car_taxi_charge": transport["total"],
                },
            )
            clearance = "enable"
        ...
        return HttpResponseRedirect(reverse("suspense:clearance_search"))
...
def with_transport(request):
    ...
    if lab_staff_list[0]:
        for lab_staff_value in lab_staff_list:
            lab_temp = []
            lab_staff_obj = Staff.objects.values("name", "position__position").filter(
                code=lab_staff_value
            )[0]
    ...
    if field_staff_list[0]:
        for field_staff_value in field_staff_list:
            field_temp = []
            field_staff_obj = Staff.objects.values("name", "position__position").filter(
                code=field_staff_value
            )[0]
    ...
    distribution = Distribution.objects.values("name").get(ratio=voucherid["ratio"])
    ...
    sus_cleared_reg = SuspenseClearedRegister.objects.filter(
        voucher_no=number, session_id=session
    )[0]
    header = HeaderFooter.objects.values("header").get(is_active=True)
    return render(
        request,
        "suspense/with_transport.html",
        {
            "header": header,
            "voucher_no": number,
...
def other_charges(request):
    ...
    financialsession = FinancialSession.objects.values("id").get(id=session)
    try:
        transport = Transport.objects.values("id", "date_of_generation", "total").get(
            voucher_no=number, session=financialsession["id"]
        )
        transport_total = transport["total"]
        transportbillno = TransportBillOfSession.objects.values(
            "transportbillofsession"
        ).get(transport__voucher_no=number, transport__session=session)
    except:
        transport_total = 0
        transport = 0
        transportbillno = 0
    ...
    if car_taxi_advance_obj:
        car_taxi_advance = CarTaxiAdvance.objects.values(
            "spent", "advance", "balance", "receipt_no", "receipt_session"
        ...
        receipt_dated = VoucherId.objects.values("purchase_order__date_time").filter(
            ...
            session_id=car_taxi_advance["receipt_session"],
        )[0]
        return render(
            request,
            "suspense/othercharge.html",
            {
                "header": header,
                "voucher_no": number,
                "date": suspenseclearance["clear_date"],
...
def quoted_add_distance(request):
    ...
    for item in items:
        if item:
            parents.append(
                QuotedItem.objects.values("item__category__parent__name", "id")
                .filter(item=item)
    ...
    for parent in parents:
        ...
        for category in parent:
            ...
            if (
                ...
                or value.split(":")[1].upper() == " FIELD WORK"
            ):
                field_work.append(key)
    if field_work and generate_voucher == 1:
        if request.method == "POST":
            ...
            return HttpResponseRedirect(reverse("bills:quoted_bill_cal"))
        else:
            return render(
                request,
                "suspense/quoted_add_distance.html",
        ...
        if form.is_valid():
            ...
            if object:
                ...
                messages = (
                    ...
                    + " and Session"
                    + " "
                    + str(session_start_date)
                    + ":"
                    + str(session_end_date)
                )
                request_status = request_notify()
                temp = {
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if not "session" in request.POST:
                HttpResponseRedirect(reverse("suspense:sessionselect"))
            ...
            rate_object = Surcharge.objects.filter(tax_name="transportation").values(
                "value"
            )[0]
            ...
            total = rate * distance
            suspense_object = SuspenseOrder.objects.filter(
                voucher=voucher, session_id=session
            ).update(is_cleared=0)
            try:
                if Transport.objects.filter(
                    ...
                    max_id = TransportBillOfSession.objects.all().aggregate(Max("id"))
                    if max_id["id__max"] == None:
                        ...
                        transportbillofsession_obj = TransportBillOfSession.objects.values(
                            "transportbillofsession", "session"
                        ).get(
                            id=max_id["id__max"]
                        )
                        if transportbillofsession_obj["session"] == session_id["id"]:
                            temp_obj = TransportBillOfSession(
                                transport=transport_obj,
                                session=session,
                                transportbillofsession=transportbillofsession_obj[
                                    "transportbillofsession"
                                ]
                                + 1,
                            )
                ...
                pass
            temp = list(
                Transport.objects.filter(voucher_no=voucher, session=session).values()
            )
            total_amount = (
            ...
            return render(
                request,
                "suspense/transport_summary.html",
                {
...
def tada_result(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for testing_staff_var in testing_staff_list:
                testing_staff_details = Staff.objects.filter(
                    code=testing_staff_var
                ).values("name", "daily_ta_da")
            ...
            footer = HeaderFooter.objects.values("footer").get(is_active=True)
            voucher_obj = (
                VoucherId.objects.filter(session=session)
            ...
            purchase_order_object = PurchaseOrder.objects.filter(
                id=purchase_order_var
            ).values(
                "id",
                "buyer_id__username",
                "buyer_id__first_name",
                "buyer_id__last_name",
            )
            tada_total = 0
            ...
            object = list(
                TaDa.objects.filter(
                    session=session, voucher_no=voucher, start_test_date=start_test_date
                ).values()
            )
            if object:
                TaDa.objects.filter(
                    ...
                    tada_amount=tada_total_with_tax,
                    tada_amount_without_tax=tada_total,
                ...
                for tax in tada_taxes:
                    tada_tax_detail = TaDa_Tax_Detail(
                        tada=obj, name=tax[0], amount=tax[1]
                    )
                    tada_tax_detail.save()
            recent_tada = (
                TaDa.objects.values_list("id", flat=True)
                .filter(voucher_no=voucher)
                .order_by("-id")[0]
            )
            tada_obj = TaDa.objects.values(
                "departure_time_from_tcc",
                "arrival_time_at_site",
                "departure_time_from_site",
                "arrival_time_at_tcc",
                "tada_amount",
                "start_test_date",
                "end_test_date",
            ...
            return render(
                request,
                "suspense/tada_summary.html",
                {
                    "purchase_order_object": purchase_order_object,
                    "tada": tada_obj,
                    "purchase_order_id": voucher,
                    "list_staff": list_staff,
                    "words": num2eng(int(tada_total_with_tax)),
                    "total": tada_total_with_tax,
                    "amount": tada_total,
                    "tax_detail": tada_tax_detail_list,
                    "request": request_status,
                    "session": session,
                    "voucher": voucher,
                },
            ...
            voucher = request.POST["voucher_no"]
            form = TaDaForm(
                request.POST, initial={"voucher_no": voucher, "session": session}
            )
            message = "Fields are mandatory"
            tada = "enable"
            request_status = request_notify()
        ...
        if request.GET["voucher_no"]:
            ...
            return HttpResponseRedirect(reverse("suspense:tada_order_session"))
...
def tada_order_session(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            request_status = request_notify()
            return render(
                request, "suspense/form.html", {"form": form, "request": request_status}
            )
    else:
        form = SessionSelectForm()
        request_status = request_notify()
    ...
    if request.method == "POST":
        ...
        if date_form.is_valid():
            ...
            suspense_cleared = SuspenseOrder.objects.filter(
                purchase_order__date_time__range=(start_date, end_date)
            ).values("voucher", "session_id", "is_cleared")
            list_clearance = []
            list_user = []
            list_details = []
            for suspense_var in suspense_obj:
                SuspenseClearance_object = (
                    SuspenseClearance.objects.filter(session=suspense_var["session_id"])
            ...
            for temp_var in list_clearance:
                for voucher_var in temp_var:
                    voucher_object = (
                        VoucherId.objects.filter(voucher_no=voucher_var["voucher_no"])
            ...
            return render(
                request,
                "suspense/mark_suspense_clear.html",
                {
                    "listed": list_details,
                    "suspense_cleared": suspense_cleared,
                    "request": request_status,
                },
            )
...
def mark_status(request):
    ...
    try:
        est_transport = SuspenseOrder.objects.values("distance_estimated").get(
            voucher=voucher, session_id_id=session
        )
        delivery_rate = Surcharge.objects.values("value").filter(
            tax_name="Transportation"
        )[0]
        est_transport_total = (
            est_transport["distance_estimated"] * delivery_rate["value"]
        )
    except:
    ...
    distribution_total = (
        suspense_total - boring_charge_internal - other_charges - tada_amount
    )
    voucherid = VoucherId.objects.values(
        "ratio",
        "college_income",
        "admin_charges",
        "purchased_item__item__category__parent__name",
    ).filter(voucher_no=voucher, session_id=session)[0]
    temp_val = voucherid["purchased_item__item__category__parent__name"].split(":")
    try:
        if temp_val[1].upper() == "FIELD WORK" or temp_val[1].upper() == " FIELD WORK":
    ...
    remain_cost = distribution_total - work_charge - college_income - admin_charges
    ...
    calculate_distribution_total = (
        college_income + admin_charges + consultancy_asset + development_fund
    )
    CalculateDistribution.objects.filter(voucher_no=voucher, session_id=session).update(
        college_income_calculated=college_income,
        admin_charges_calculated=admin_charges,
        consultancy_asset=consultancy_asset,
        development_fund=development_fund,
        total=calculate_distribution_total,
    )
    SuspenseClearance.objects.filter(voucher_no=voucher, session_id=session).update(
        work_charge=work_charge
    )
    suspense_order_obj = (
        SuspenseOrder.objects.filter(voucher=voucher)
        .filter(session_id=session)
        .update(is_cleared="1")
    )
    if SuspenseClearedRegister.objects.filter(
        voucher_no=voucher, session=session
    ).exists():
        pass
    else:
        ...
        max_id = SuspenseClearedRegister.objects.all().aggregate(Max("id"))
        if max_id["id__max"] == None:
            temp_obj = SuspenseClearedRegister(
                suspenseclearednumber=1, session=financialsession, voucher_no=voucher
            )
            temp_obj.save()
        else:
            suspenseclearednumber_obj = SuspenseClearedRegister.objects.values(
                "suspenseclearednumber", "session"
...
def clearance_options(request):
    ...
    voucherid = VoucherId.objects.values(
        "purchase_order_of_session", "purchase_order"
    ).filter(voucher_no=voucher_no, session_id=session_id)[0]
    with_transport = 0
    try:
    ...
    details = PurchaseOrder.objects.values(
        ...
        "cheque_dd_number",
        "cheque_dd_date",
    ).filter(id=voucherid["purchase_order"])[0]
    request_status = request_notify()
    return render(
        request,
        "suspense/clearance_options.html",
        {
            "details": details,
...
def summary_page(request):
    ...
    try:
        ...
        tada_amount = 0
    transport = (
        Transport.objects.values("rate", "total")
        .filter(voucher_no=voucher)
        .filter(session=session)[0]
    )
    distance_travelled = transport["total"] / transport["rate"]
    other_charges = list(
        SuspenseClearance.objects.filter(voucher_no=voucher)
...
def transport_bill(request):
    ...
    if request.method == "POST":
        session = request.POST["session"]
        voucher = request.POST["voucher"]
        bill_no = TransportBillOfSession.objects.values("transportbillofsession").get(
            transport__voucher_no=voucher, transport__session=session
        )
        transport_object = Transport.objects.filter(
            session_id=session, voucher_no=voucher
        ).values(
...
def tada_bill_list(request):
    ...
    voucher = request.POST["voucher"]
    tada = (
        TaDa.objects.values("id", "tada_amount", "start_test_date", "end_test_date")
        .filter(voucher_no=voucher)
        .filter(session=session)
    )
...
def tada_bill(request):
    ...
    if start_test_date == end_test_date:
        ...
        days = no_of_days + 1
    for testing_staff_var in testing_staff_list:
        testing_staff_details = Staff.objects.filter(code=testing_staff_var).values(
            "name", "daily_ta_da"
        )
        for tada_val in testing_staff_details:
            tada_val["daily_ta_da"] = tada_val["daily_ta_da"] * days
    ...
    voucher_obj = VoucherId.objects.values(
        "purchase_order_of_session", "receipt_date"
    ).filter(session=tada_obj.session, voucher_no=tada_obj.voucher_no)[0]
    purchase_order_var = 0
    purchase_order_var = voucher_obj["purchase_order_of_session"]
    purchase_order_object = PurchaseOrder.objects.filter(
        voucherid__purchase_order_of_session=purchase_order_var,
        voucherid__session_id=tada_obj.session,
    ).values(
        "buyer_id__first_name",
        "buyer_id__last_name",
        "buyer__customer__title",
    ...
    return render(
        request,
        "suspense/tada_result.html",
        {
            "purchase_order_object": purchase_order_object,
            "tada": tada_object,
            "purchase_order_id": purchase_order_var,
            "words": num2eng(int(tada_amount)),
            "tada_total": tada_amount,
            "tada_amount": tada_amount_without_tax,
            "tax_detail": tada_tax_detail_list,
            "request": request_status,
            "session": tada_obj.session,
            "voucher": tada_obj.voucher_no,
            "list_staff": list_staff,
            "header": header,
...
def car_taxi_advance_form(request):
    if request.method == "POST":
        ...
        if sessiondata.is_valid():
            ...
            if object:
                form = CarTaxiAdvance_form(
                    initial={"voucher_no": voucher, "session": session}
                )
                car_taxi_advance = "enable"
                request_status = request_notify()
                return render(
                    request,
                    "suspense/car_taxi_advance_form.html",
                    {
                        "form": form,
                        "request": request_status,
                        "car_taxi_advance": car_taxi_advance,
...
def car_taxi_advance(request):
    if request.method == "POST":
        ...
        if car_taxi_advance_form.is_valid():
            ...
            return render(
                request,
                "suspense/car_taxi_advance_success.html",
                {"request": request_status},
            )
        else:
        ...
        return HttpResponseRedirect(reverse("suspense:car_taxi_advance_form"))
