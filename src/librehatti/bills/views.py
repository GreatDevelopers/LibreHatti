    ...
    for value in surcharge:
        ...
        if surcharge_tax == 1 and generate_tax == 1:
            ...
            taxes_applied = QuotedTaxesApplied(quoted_order=quoted_order,
                                               ...
                                               tax=taxes, surcharge_name=value[
                    'tax_name'], surcharge_value=value['value'])
            taxes_applied.save()
...
def select_note(request):
    ...
    return render(request, 'bills/select_note.html', {'form': form,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse(
                "librehatti.bills.views.quoted_order_added_success"))
        else:
            return HttpResponseRedirect(reverse(
                "librehatti.bills.views.quoted_order_added_success"))
    else:
...
def quoted_order_of_session(request):
    ...
    quoted_order_id = request.session.get('quoted_order_id')
    quoted_order = QuotedOrder.objects.get(id=quoted_order_id)
    quoted_order_obj = QuotedOrder.objects.values('id', 'date_time').get(
        id=quoted_order_id)
    ...
    if max_id['id__max'] is None:
        obj = QuotedOrderofSession(quoted_order=quoted_order, session=session,
                                   quoted_order_session=1)
        obj.save()
    else:
        quoted_order_of_session = QuotedOrderofSession.objects.values(
            'quoted_order_session', 'session').get(id=max_id['id__max'])
    ...
    request.session['quoted_order_id'] = quoted_order_id
    return HttpResponseRedirect(reverse(
        "librehatti.suspense.views.quoted_add_distance"))
