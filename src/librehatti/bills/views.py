...
def quoted_order_added_success(request):
    ...
    """
    quoted_order_id = request.session.get("quoted_order_id")
    details = QuotedOrder.objects.values(
        "buyer__first_name",
        "buyer__last_name",
    ...
    return render(
        request,
        "bills/quoted_success.html",
        {"details": details, "quoted_order_id": quoted_order_id},
    )
...
def quoted_order_of_session(request):
    ...
    request.session["old_post"] = old_post
    request.session["quoted_order_id"] = quoted_order_id
    return HttpResponseRedirect(reverse("suspense:quoted_add_distance"))
