    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            total_cost = PurchasedItem.objects.filter(quote_order_id=
                         int(client_id)).aggregate(Sum('price')).get('price__sum',
                         0.00)
            return render(request, 'bills/confirm.html',{'total_cost':total_cost, 
                          'form':form })
        else:
             ...
             total_cost = QuotedItem.objects.filter(quote_order_id=
                          int(client_id)).aggregate(Sum('quote_price')).get(
                         'price__sum',0.00)
             i_d = quoted_order.quote_buyer_id_id
