...
class DailyReportForm(forms.Form):
    ...
    mode_of_payment = forms.ChoiceField(
        choices=[('1', 'Cash'), ('2', 'DD/Cheque/RTGS')])

    def __init__(self, *args, **kwargs):
        super(DailyReportForm, self).__init__(*args, **kwargs)
        self.fields['mode_of_payment'].widget.attrs = {
...
class ConsultancyFunds(forms.Form):
    ...
    parent_category = forms.ModelChoiceField(
        queryset=Category.objects.filter(parent__parent__isnull=True).filter(
            parent__isnull=False), label=_PARENT_CATEGORY)
    try:
        ...
        sub_category_choices = [('', '--------')] + [(
            id, name) for id, name in itertools.izip(
            sub_category_id, sub_category_name)]
        sub_category = forms.MultipleChoiceField(
            sub_category_choices, label=_SUB_CATEGORY)
    except:
        pass
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['sub_category'].widget.attrs = {
            'class': 'btn btn-default dropdown-toggle', 'size': '15'}
...
class ClientForm(forms.Form):
    ...
    """
    client_fields = forms.MultipleChoiceField(
        required=False, widget=forms.CheckboxSelectMultiple,
        choices=CLIENT_FIELD_CHOICES)
...
class AddConstraints(forms.Form):
    ...
    start_date = forms.DateField(required=False, initial=str(
        datetime.date.today().year) + '-04-01')
    end_date = forms.DateField(required=False, initial=datetime.date.today())
    year = forms.ChoiceField(choices=[(datetime.date.today().year,
                                       datetime.date.today().year)] + [
        (x, x) for x in range(2000, 2050)], required=False)
    ...
    session_choices = [('', '--------')] + [(id, str(start) + '-To-' + str(
        end)) for id, start, end in itertools.izip(session_id, session_start,
                                                   session_end)]
    session = forms.ChoiceField(session_choices)

    surcharges = forms.ModelMultipleChoiceField(
        required=False, widget=forms.CheckboxSelectMultiple,
        queryset=Surcharge.objects.filter(taxes_included=True))
...
class PaidTaxesForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for val in surcharges:
            self.fields['paid_' + val.tax_name.replace(
                " ", "_").lower()] = forms.IntegerField(initial=0)
            self.fields['paid_' + val.tax_name.replace(
                " ", "_").lower()].widget.attrs = { 'class': 'form-control'}
...
class LabReportForm(forms.Form):
    ...
    parent_category = forms.ModelChoiceField(queryset=Category.objects.filter(
