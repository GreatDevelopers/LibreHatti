...
class AddConstraints(forms.Form):
    ...
    start_date = forms.DateField(
        required=False, initial=str(datetime.date.today().year) + "-04-01"
    )
    end_date = forms.DateField(required=False, initial=datetime.date.today())
    year = forms.ChoiceField(
        choices=[(datetime.date.today().year, datetime.date.today().year)]
        + [(x, x) for x in range(2000, 2050)],
        required=False,
    )
