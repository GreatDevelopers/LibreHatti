...
def daily_report_form(request):
    """
    This view is used to render the form for daily report registers
    """
...
def daily_report_result(request):
    ...
    if 'button1' in request.POST:
        ...
        if form.is_valid():
            ...
            list_of_report = []
            purchase_order = PurchaseOrder.objects.filter(mode_of_payment = mode_of_payment).values('date_time','id')
            for date_value in purchase_order:
                if  start_date <= str(date_value['date_time']) <= end_date:
