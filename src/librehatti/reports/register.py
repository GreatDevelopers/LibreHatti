...
from librehatti.reports.forms import DailyReportForm, ConsultancyFunds, \
    DateRangeSelectionForm, MonthYearForm, AmountForm, PaidTaxesForm, \
    LabReportForm
...
from librehatti.catalog.models import PurchaseOrder, Bill, PurchasedItem, \
    TaxesApplied, Surcharge, NonPaymentOrder, NonPaymentOrderOfSession, \
    ModeOfPayment, Category

from librehatti.suspense.models import SuspenseOrder, Transport, TaDa, Staff, \
    SuspenseClearance, SuspenseClearedRegister

from librehatti.voucher.models import CalculateDistribution, VoucherId, \
    VoucherTotal, Distribution

from librehatti.bills.models import QuotedOrder, QuotedTaxesApplied, QuotedItem
        ...
        if form.is_valid() and date_form.is_valid():
            ...
            if mode_of_payment == '1':
                purchase_order = PurchaseOrder.objects.filter(
                    date_time__range=(start_date,end_date)).filter(
                    mode_of_payment__method='Cash').values(
                    'date_time', 'bill__amount_received',
                    'voucherid__purchase_order_of_session','buyer__first_name',
                    'buyer__last_name', 'buyer__customer__address__pin',
                ...
                purchase_order = PurchaseOrder.objects.filter(
                    date_time__range=(start_date,end_date)).exclude(
                    mode_of_payment__method='Cash').values('date_time',
            ...
            for temp_value in purchase_order:
                ...
                if temp_value['buyer__first_name']:
                    name = temp_value['buyer__first_name'] +" "+ temp_value[
                        'buyer__last_name']
                else:
            ...
            start_date = datetime.strptime(
                start_date, '%Y-%m-%d').strftime('%B-%d-%Y')
            end_date = datetime.strptime(
                end_date, '%Y-%m-%d').strftime('%B-%d-%Y')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and date_form.is_valid():
            ...
            voucher_object = VoucherId.objects.filter(
                ...
                purchased_item__item__category__in = category,
                is_special=0).values('purchase_order_of_session',
                                     'voucher_no', 'session_id',
            ...
            for temp_value in voucher_object:
                ...
                if temp_value['purchase_order__buyer__first_name']:
                    name = temp_value[
                        'purchase_order__buyer__first_name'] +" "+ temp_value[
                        'purchase_order__buyer__last_name']
                else:
                ...
                consultancy_var = CalculateDistribution.objects.values(
                    'consultancy_asset').get(voucher_no=
                ...
                consultanttotal = consultanttotal + consultancy_var[
                    'consultancy_asset']
                result.append(temp_list)
                temp_list = []
            ...
            end_date = datetime.strptime(end_date, '%Y-%m-%d').strftime(
                '%B-%d-%Y')
            back_link=reverse(
                'librehatti.reports.register.consultancy_funds_report')
            return render(request, 'reports/consultancy_funds_result.html',
             {'result':result, 'back_link':back_link, 'start_date':start_date,
              'end_date':end_date, 'sum':sum, 'category_name':category_name,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            taxesapplied_obj = TaxesApplied.objects.values(
                'purchase_order__id').filter(purchase_order__date_time__range=(
                start_date,end_date), purchase_order__tds__gt=0)
            bill_object = Bill.objects.filter(
                purchase_order__in=taxesapplied_obj).values(
                'purchase_order__date_time','purchase_order__id',
                'purchase_order__buyer__first_name',
                'purchase_order__buyer__last_name',
                'purchase_order__buyer__customer__title',
                ...
                'grand_total','purchase_order__buyer__customer__user__email',
                'purchase_order__buyer__customer__telephone')
            servicetax = 0
            Heducationcess = 0
            educationcess = 0
            ...
            for temp_value in bill_object:
                flag = 1
                voucher_object = VoucherId.objects.filter(
                    purchase_order_id = temp_value[
                        'purchase_order__id']).values(
                    'purchase_order_of_session').distinct()
            ...
            tax = TaxesApplied.objects.filter(
                purchase_order__date_time__range=(start_date,end_date),
                purchase_order__tds__gt=0).values('surcharge','tax')
            for value in tax:
            ...
            {'result':result,'request':request_status,'servicetax':servicetax,
             'Heducationcess':Heducationcess, 'educationcess':educationcess,
             'start_date':start_date,'grandtotal':grandtotal,'end_date':end_date,
        ...
        if form.is_valid():
            ...
            for temp_value in bill_object:
                ...
                voucher_object = VoucherId.objects.filter(
                    purchase_order_id = temp_value[
                        'purchase_order__id']).values(
                    'purchase_order_of_session','voucher_no').distinct()
                for value in voucher_object:
                ...
                material = PurchasedItem.objects.values(
                    'item__category__name').filter(
                    purchase_order__id = temp_value[
                        'purchase_order__id']).distinct()
                for item in material:
                    if flag == 1:
                        material_list = item['item__category__name']
                        flag = 0
                    else:
                       material_list = material_list + ', ' + item[
            ...
            tax = TaxesApplied.objects.filter(purchase_order__date_time__range=(
                start_date,end_date)).values('surcharge','tax')
            for value in tax:
                list_of_taxes.append(value)
            ...
            'end_date':end_date,'billamount':billamount, 'tds':tds,
        ...
        if form.is_valid():
            ...
            for value in suspenseclearance:
                ...
                suspense = SuspenseOrder.objects.values('voucher',
                    ...
                    'purchase_order__buyer__customer__address__pin',
                    'purchase_order__buyer__customer__address__province').\
                filter(is_cleared=1, voucher=value[
                    'voucher_no'],session_id=value['session_id'])[0]
                temp.append(suspense['voucher'])
                cleared_voucher_no = SuspenseClearedRegister.objects.values(
                    'suspenseclearednumber').filter(
                    voucher_no=suspense['voucher'], session_id=suspense[
                        'session_id'])[0]
                ...
                if suspense['purchase_order__buyer__first_name']:
                    ...
                    if suspense[
                    'purchase_order__buyer__customer__address__pin'] is None:
                        address =suspense[
                                     'purchase_order__buyer__customer__title']\
                        + ', ' +suspense[
                        'purchase_order__buyer__customer__address__street_address']\
                ...
                filter(voucher_no=suspense['voucher'], session_id=suspense[
                    'session_id'])[0]
                ...
                try:
                    ...
                    grand_total = caldistribute['total'] + trans_value + \
                                  tada_value + suspensecl['work_charge'] + \
                                  other_charges + \
...
def servicetax_register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and data_form.is_valid():
            ...
            total = 0
            totalplustax = 0
            taxes_name = TaxesApplied.objects.values(
                'surcharge_name','surcharge_value').filter(
                purchase_order__date_time__month=month,
            ...
            for val in surcharges:
                for val2 in taxes_name:
                    if val.tax_name in val2['surcharge_name']:
                        paid_taxes['%s' % val.tax_name.replace(
                            " ", "_").lower()] = data_form.cleaned_data[
                            'paid_' + val.tax_name.replace(" ", "_").lower()]
                        init_taxes[
                            '%s' % val.tax_name.replace(" ", "_").lower()] = 0
            taxesapplied_obj = TaxesApplied.objects.values(
                'purchase_order__id').filter(
                purchase_order__date_time__month=month,
                purchase_order__date_time__year=year).order_by(
                'purchase_order__date_time')
            purchase_order = PurchaseOrder.objects.values('date_time', 'id',
                ...
                'buyer__customer__address__province').filter(
                id__in=taxesapplied_obj).order_by('date_time')
            result = []
            for value in purchase_order:
                ...
                if value['buyer__first_name']:
                    address = value['buyer__first_name'] +\
                              ...
                              value['buyer__customer__address__street_address']\
                              + ', ' + value[
                        'buyer__customer__address__district'] + ', ' + \
                        value['buyer__customer__address__province']
                else:
                    address = value['buyer__customer__title'] + ', ' +\
                              value['buyer__customer__address__street_address'] \
                              + ', ' + value[
                        'buyer__customer__address__district'] + ', ' +\
                              value['buyer__customer__address__province']
                ...
                for val in taxes_name:
                    taxesapplied = TaxesApplied.objects.values(
                        'tax', 'surcharge_name').filter(
                        purchase_order=value['id'], surcharge_name=val[
                            'surcharge_name'])
                    if taxesapplied:
                        taxesapplied = TaxesApplied.objects.values(
                            'tax', 'surcharge_name').filter(
                            purchase_order=value['id'], surcharge_name=val[
                                'surcharge_name'])[0]
                        init_taxes[val['surcharge_name'].replace(
                            " ", "_").lower()] = init_taxes[val[
                            'surcharge_name'].replace(
                            " ", "_").lower()] + taxesapplied['tax']
            ...
            for val in surcharges:
                for val2 in taxes_name:
                    if val.tax_name in val2['surcharge_name']:
                        not_paid_taxes['%s' % val.tax_name.replace(
                            " ", "_").lower()] = init_taxes[
            ...
            'not_paid_taxes':not_paid_taxes, 'paid_taxes':paid_taxes,
...
def main_register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            year = request.POST['year']
            list_of_client = []
            suspense_order = SuspenseOrder.objects.values_list(
                'purchase_order', flat=True).filter(
                purchase_order__date_time__month=month).filter(
                purchase_order__date_time__year=year)
            purchase_order = PurchaseOrder.objects.filter(
                date_time__month = month).filter(
                date_time__year = year).values(
                'voucherid__purchase_order_of_session', 'voucherid__session',
                'voucherid__voucher_no', 'date_time', 'buyer__first_name',
                'buyer__last_name', 'buyer__customer__address__pin',
                'buyer__customer__title',
                'buyer__customer__address__street_address',
                'buyer__customer__address__district',
                'buyer__customer__address__province',
                ).exclude(id__in = suspense_order).exclude(
                voucherid__is_special=1).distinct().order_by('date_time',
                'voucherid__receipt_no_of_session')
            distribution = Distribution.objects.values('college_income',
                'admin_charges').filter()[0]
            temp_list = []
            ...
            for temp_value in purchase_order:
                ...
                temp_list.append(temp_value[
                        ...
                        + ', ' + temp_value[
                        'buyer__customer__address__province'])
                material = VoucherId.objects.values(
                ...
                calculated_distribution = CalculateDistribution.objects.filter(
                    voucher_no=temp_value['voucherid__voucher_no']).filter(
                    session = temp_value['voucherid__session']).values(
                    'college_income_calculated','admin_charges_calculated',
                    'consultancy_asset','development_fund','total')[0]
                temp_list.append(calculated_distribution[
                ...
                developmenttotal = developmenttotal + calculated_distribution[
                    'development_fund']
...
def proforma_register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            start_date = request.POST['start_date']
            end_date = request.POST['end_date']
            quotedtaxesapplied = QuotedTaxesApplied.objects.values(
                'quoted_order__id').filter(
            ...
            for order in quotedorder:
                ...
                if order['buyer__first_name']:
                    name = order['buyer__first_name'] + ' ' + order[
                        'buyer__last_name']
                else:
                ...
                quoteditem = QuotedItem.objects.values('item__category__name',
                    'item__category__id').filter(
                    quoted_order=order['id']).distinct()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for order in non_payment_order:
                ...
                values('non_payment_order_of_session').get(
                    non_payment_order=order['id'])
                temp.append(nonpaymentorderofsession[
...
def client_register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            list_of_client = []
            purchase_order = PurchaseOrder.objects.filter(
                date_time__range=(start_date,end_date)).values( 'date_time',
                'buyer__first_name', 'buyer__last_name',
                'buyer__customer__address__pin',
                'buyer__customer__address__street_address',
                'buyer__customer__address__district',
                'buyer__customer__address__province',
                'buyer__customer__telephone','buyer__customer__user__email',
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and date_form.is_valid():
            ...
            purchase_item = PurchasedItem.objects.filter(
                purchase_order__date_time__range=(start_date, end_date),
                item__category__in=category).values('purchase_order_id',
                'purchase_order__date_time', 'price',
                'purchase_order__buyer__first_name',
                'purchase_order__buyer__last_name',
                'purchase_order__buyer__customer__title',
                'purchase_order__buyer__customer__company',
                    ...
                    try:
                        taxes_applied = TaxesApplied.objects.filter(
                            purchase_order=value[
                                'purchase_order_id'], surcharge_id=tax.id)[0]
            ...
            total = PurchasedItem.objects.filter(purchase_order__date_time__range
                = (start_date,end_date),item__category__in=category).aggregate(
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            suspenseorder = SuspenseOrder.objects.filter(
                ...
                'distance_estimated').order_by(
                'purchase_order__date_time','voucher')
...
def lab_report(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and date_form.is_valid():
            ...
            total = PurchasedItem.objects.filter(purchase_order__date_time__range
                = (start_date,end_date),
                    item__category__parent=category).aggregate(
                Sum('price')).get('price__sum', 0.00)
            request_status = request_notify()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            suspenseorder = SuspenseOrder.objects.values('voucher',
                ...
                'purchase_order__buyer__customer__address__district',
                'purchase_order__buyer__customer__address__pin',
                'purchase_order__buyer__customer__address__province').filter(
                is_cleared=0, purchase_order__date_time__range=(
                    start_date,end_date))
            result = []
        ...
        if form.is_valid():
            ...
            for voucher in suspense_cleared:
                ...
                if voucherid['purchase_order__buyer__first_name']:
                    ...
                    if voucherid[
                        'purchase_order__buyer__customer__address__pin'] is \
                            None:
                        address =voucherid[
                                     'purchase_order__buyer__customer__title']\
                                 + ', ' + \
